 ...
 */
public class PruefungLaufendesEntgeltAv extends AbstractFeldPruefung<FeldNameDBKV, FehlerNummerDBKV> {
	 ...
	 */
	public PruefungLaufendesEntgeltAv(final Feld<FeldNameDBKV> feld, final Feld<FeldNameDBKV> feldZeitraumBeginn, final Feld<FeldNameDBKV> feldZeitraumEnde, final Feld<FeldNameDBKV> feldKennzRk, final Baustein<FeldNameDSME> bausteinDsme) {
		super(feld);
	...
	public void initialisierePruefungen() throws UngueltigeDatenException {
		final PruefungNumerisch kv180 = new PruefungNumerisch(getFeld(), true);
		addPruefung(kv180, new Fehler<FehlerNummerDBKV>(FehlerNummerDBKV.DBKV180));
	}
	...
	public void initialisiereFeldUebergreifendePruefungen() throws UngueltigeDatenException {
		...
		if (isPruefbar(FehlerNummerDBKV.DBKV184, feldZeitraumBeginn) && zeitraumBeginn.before(Anfang_2015)) {
			final PruefungInList kv184 = new PruefungInList(GRUNDSTELLUNG, getFeld());
			addPruefungFeldUebergreifend(kv184, new Fehler<FehlerNummerDBKV>(FehlerNummerDBKV.DBKV184));
		}
	}
	...
	public void initialisiereBausteinUebergreifendePruefungen() throws UngueltigeDatenException {
		...
		if (isPruefbar(FehlerNummerDBKV.DBKV182, feldZeitraumBeginn, feldZeitraumEnde, feldKennzRk, feldDsmeBbnrvu, feldDsmeVstr)) {
			...
			try {
				...
				final PruefungKleinerGleich kv182 = new PruefungKleinerGleich(getFeld(), geltendeBezugsgr.doubleValue(), 2);
				addPruefungBausteinUebergreifend(kv182, new Fehler<FehlerNummerDBKV>(FehlerNummerDBKV.DBKV182));

			} catch (final WertelistenException e) {
				final PruefungFuegeFehlerHinzu kv182 = new PruefungFuegeFehlerHinzu(getFeld());
				this.addPruefungBausteinUebergreifend(kv182, new Fehler<FehlerNummerDBKV>(FehlerNummerDBKV.DBKV182));
			}
		}
	}
}
