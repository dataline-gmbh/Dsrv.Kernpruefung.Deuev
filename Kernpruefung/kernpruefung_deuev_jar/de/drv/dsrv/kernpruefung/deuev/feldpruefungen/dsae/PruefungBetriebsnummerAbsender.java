...
/**
 * Pruefung fuer das Feld Betriebsnummer Absender aus dem Baustein DSAE.
 */
public class PruefungBetriebsnummerAbsender extends AbstractFeldPruefung<FeldNameDSAE, FehlerNummerDSAE> {

	private static final List<String> VFMM_BATRV = Arrays.asList("76641777", "12621621");
	private static final List<String> VFMM_UETBF = Arrays.asList("98503184", "98702232");
	 ...
	 */
	public PruefungBetriebsnummerAbsender(final Feld<FeldNameDSAE> feld, final String vfmm) {
		super(feld);
		this.vfmm = vfmm;
	}
	...
	public void initialisierePruefungen() throws UngueltigeDatenException {
		final PruefungBetriebsnummer ae020 = new PruefungBetriebsnummer(getFeld());
		addPruefung(ae020, new Fehler<FehlerNummerDSAE>(FehlerNummerDSAE.DSAE020));

		if (vfmm.compareTo("BATRV") == 0) {
			final PruefungInList ae022 = new PruefungInList(VFMM_BATRV, getFeld(), true);
			addPruefung(ae022, new Fehler<FehlerNummerDSAE>(FehlerNummerDSAE.DSAE022));
		}

		else if (vfmm.compareTo("UETBF") == 0) {
			final PruefungInList ae022 = new PruefungInList(VFMM_UETBF, getFeld(), true);
			addPruefung(ae022, new Fehler<FehlerNummerDSAE>(FehlerNummerDSAE.DSAE022));
		} 
